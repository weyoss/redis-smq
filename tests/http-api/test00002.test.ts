...
test('Messages HTTP API: Case 2', async () => {
  ...
  const response1: IResponse = await request.get('/api/messages?skip=0&take=2');
  ...
  const response2: IResponse = await request.get('/api/messages?skip=2&take=2');
  ...
  const response3: IResponse = await request.get('/api/messages?skip=4&take=2');
