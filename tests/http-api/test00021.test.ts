...
test('Purge pending messages', async () => {
  ...
  const response1: ISuperTestResponse<GetMessagesResponseBodyDataDTO> =
    await request.get(
      `/api/queues/${producer.getQueueName()}/pending-messages?skip=0&take=99&ns=${redisKeys.getNamespace()}`,
    );
  ...
  const response2: ISuperTestResponse<GetMessagesResponseBodyDataDTO> =
    await request.delete(
      `/api/queues/${producer.getQueueName()}/pending-messages?ns=${redisKeys.getNamespace()}`,
    );
