...
export class ExchangeFanOutService {
  ...
  async unbindQueue(queueParams: IQueueParams, fanOutName: string) {
    await this.exchangeFanOut.unbindQueueAsync(queueParams, fanOutName);
  }
  ...
  async deleteExchange(fanOutName: string) {
    return this.exchangeFanOut.deleteExchangeAsync(fanOutName);
  }
}
