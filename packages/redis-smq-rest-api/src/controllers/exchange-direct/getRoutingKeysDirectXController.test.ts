...
describe('getRoutingKeysDirectXController', () => {
  it('HTTP 200 OK', async () => {
    ...
    const response1: TResponse<GetRoutingKeysDirectXControllerResponseDTO> =
      await request.get(
        `/api/v1/namespaces/${exchange.ns}/exchanges/direct/${exchange.name}/routing-keys`,
      );
    expect(response1.status).toEqual(200);
    expect(response1.body?.data).toEqual(['rk1']);
  });
});
