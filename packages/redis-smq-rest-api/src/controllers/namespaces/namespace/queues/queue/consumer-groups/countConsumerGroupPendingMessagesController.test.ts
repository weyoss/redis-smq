...
describe('countConsumerGroupPendingMessagesController', () => {
  it('HTTP 200 OK', async () => {
    ...
    const response1: TResponse<CountConsumerGroupPendingMessagesControllerResponseDTO> =
      await request.get(
        `/api/namespaces/${queue.ns}/queues/${queue.name}/consumer-groups/${consumerGroup1}/messages/count`,
      );
    ...
    const response2: TResponse<CountConsumerGroupPendingMessagesControllerResponseDTO> =
      await request.get(
        `/api/namespaces/${queue.ns}/queues/${queue.name}/consumer-groups/${consumerGroup2}/messages/count`,
      );
  ...
  it('HTTP 400 BAD REQUEST: InvalidQueueParametersError', async () => {
    ...
    const response1: TResponse<CountConsumerGroupPendingMessagesControllerResponseDTO> =
      await request.get(
        `/api/namespaces/$$$/queues/@@@@/consumer-groups/******/messages/count`,
      );
  ...
  it('HTTP 400 BAD REQUEST: ConsumerGroupsNotSupportedError', async () => {
    ...
    const response1: TResponse<CountConsumerGroupPendingMessagesControllerResponseDTO> =
      await request.get(
        `/api/namespaces/${queue.ns}/queues/${queue.name}/consumer-groups/gp1/messages/count`,
      );
