...
import { DeleteConsumerGroupControllerResponseDTO } from './DeleteConsumerGroupControllerResponseDTO.js';
  ...
  it('HTTP 204 No Content', async () => {
    ...
    const response1: TResponse<DeleteConsumerGroupControllerResponseDTO> =
      await request.delete(
        `/api/namespaces/${queue.ns}/queues/${queue.name}/consumer-groups/${consumerGroup1}`,
      );
    ...
    const response2: TResponse<GetConsumerGroupsControllerResponseDTO> =
      await request.get(
        `/api/namespaces/${queue.ns}/queues/${queue.name}/consumer-groups`,
      );
