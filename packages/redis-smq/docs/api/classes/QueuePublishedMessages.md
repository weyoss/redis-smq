...
Queue string name (uses default namespace) or queue parameters object
...
When queue properties cannot be retrieved from Redis.
...
- QueueMessagesAbstract For base message browsing functionality.
- [IQueuePublishedMessagesCountByStatus](../interfaces/IQueuePublishedMessagesCountByStatus.md) For the structure of the returned counts.
- [QueuePendingMessages](QueuePendingMessages.md) For working with pending messages specifically.
- [QueueAcknowledgedMessages](QueueAcknowledgedMessages.md) For working with acknowledged messages.
- [QueueScheduledMessages](QueueScheduledMessages.md) For working with scheduled messages.
- [QueueDeadLetteredMessages](QueueDeadLetteredMessages.md) For working with dead-lettered messages.
...
Optional callback function invoked with the message counts or error. - On success: `cb(null, counts)` where `counts` is an object containing
counts for pending, acknowledged, scheduled, and dead-lettered messages. - On error: `cb(error)` with one of the errors listed below. - If not provided, the method returns a Promise that resolves with the counts
