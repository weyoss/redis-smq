[RedisSMQ](../../../README.md) / [Docs](../../README.md) / [API Reference](../README.md) / IConsumerParsedOptions

# Interface: IConsumerParsedOptions

## Extends

- `Required`\<[`IConsumerOptions`](IConsumerOptions.md)\>

## Properties

### batchAcks

> **batchAcks**: `Required`\<[`IConsumerBatchConfig`](IConsumerBatchConfig.md)\>

#### Overrides

[`IConsumerOptions`](IConsumerOptions.md).[`batchAcks`](IConsumerOptions.md#batchacks)

---

### batchUnacks

> **batchUnacks**: `Required`\<[`IConsumerBatchConfig`](IConsumerBatchConfig.md)\>

#### Overrides

[`IConsumerOptions`](IConsumerOptions.md).[`batchUnacks`](IConsumerOptions.md#batchunacks)

---

### enableMultiplexing

> **enableMultiplexing**: `boolean`

#### Inherited from

[`IConsumerOptions`](IConsumerOptions.md).[`enableMultiplexing`](IConsumerOptions.md#enablemultiplexing)

---

### heartbeatTTL

> **heartbeatTTL**: `number`

#### Inherited from

[`IConsumerOptions`](IConsumerOptions.md).[`heartbeatTTL`](IConsumerOptions.md#heartbeatttl)
