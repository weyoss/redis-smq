...
import { QueueManagerQueueExistsError } from './errors/index.js';
import {
  EQueueDeliveryModel,
  EQueueProperty,
  EQueueType,
...
export class QueueManager {
  ...
  /**
   * Save a new queue-manager with specified parameters.
   * Upon success the callback function is invoked with the created queue-manager details.
   *
   ...
   * @param queue - The name or parameters for the queue-manager.
   * @param queueType - The type of the queue-manager, defined by EQueueType.
      ...
      (client, done) => {
        ...
        client.runScript(
          ...
          (err, reply) => {
            ...
            if (reply !== 'OK') {
              const error = new QueueManagerQueueExistsError();
            ...
            withEventBus(
              ...
              (eventBus, done) => {
                ...
                eventBus.emit('queue.queueCreated', queueParams, properties);
                this.logger.info(
  ...
  /**
   * Checks if a specified queue-manager exists.
   *
   * @param queue - The name or parameters for the queue-manager.
   * @param cb - Callback function to return a boolean indicating the existence of the queue-manager.
   */
  ...
  /**
   * Deletes a specific queue-manager.
   *
   * @param queue - The name or parameters for the queue-manager to be deleted.
   * @param cb - Callback function to handle success or error.
   */
  ...
  /**
   * Retrieves the properties of a specified queue-manager.
   *
   * @param queue - The name or parameters for the queue-manager.
   * @param cb - Callback function to return the queue-manager properties or an error.
   */
  getProperties(
    queue: string | IQueueParams,
  ...
  /**
   * Retrieves the consumers for a specified queue-manager.
   *
   * This function accepts either a queue-manager name (string) or queue-manager parameters (IQueueParams)
   ...
   *
   * @param queue - A string representing the queue-manager name or an IQueueParams object with queue-manager details.
  ...
  /**
   * Retrieves the consumer IDs for a specified queue-manager.
   *
   * This function accepts either a queue-manager name (string) or queue-manager parameters (IQueueParams)
