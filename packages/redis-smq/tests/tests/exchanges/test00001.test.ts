...
describe('ExchangeDirect', () => {
  ...
  it('bindQueue: fails when exchange and queue are of different namespaces', async () => {
    await expect(
      directExchange.bindQueue(queueA, 'exchange2', rkA),
  ...
  it('bindQueue: binds queue with exact routing key and matchQueues returns it', async () => {
    ...
    await directExchange.bindQueue(queueA, exchangeA, rkA);

    // matchQueues should return queueA for rkA
    const matchedForA = await directExchange.matchQueues(exchangeA, rkA);
    expect(matchedForA).toEqual([queueA]);

    // For another key (rkB) it should not return queueA
    const matchedForB = await directExchange.matchQueues(exchangeA, rkB);
  ...
  it('bindQueue: idempotent when binding the same queue/key again', async () => {
    ...
    // Still matches
    const matched = await directExchange.matchQueues(exchangeA, rkA);
  ...
  it('bindQueue: fails when binding a non-existent queue', async () => {
    ...
    await expect(
      directExchange.bindQueue(nonExistingQueue, exchangeA, rkA),
    ).rejects.toBeTruthy();
  });

  it('matchQueues: returns empty array when no bindings match', async () => {
    await directExchange.bindQueue(queueA, exchangeA, rkA);
    const matched = await directExchange.matchQueues(exchangeA, rkUnknown);
  ...
  it('unbindQueue: removes binding so matchQueues returns empty for that key', async () => {
    ...
    await directExchange.bindQueue(queueA, exchangeA, rkA);
    const matchedBefore = await directExchange.matchQueues(exchangeA, rkA);

    expect(matchedBefore).toEqual([queueA]);

    // Unbind and verify
    await directExchange.unbindQueue(queueA, exchangeA, rkA);
    const matchedAfter = await directExchange.matchQueues(exchangeA, rkA);
  ...
  it('deleteExchange: prevents deletion when queues are bound, then deletes when unbound', async () => {
    ...
    await directExchange.bindQueue(queueB, exchangeA, rkB);

    // Try deleting while bound -> expect failure if deleteExchange is supported
    const tryDeleteWhileBound = directExchange.delete(exchangeA);
    await expect(tryDeleteWhileBound).rejects.toBeTruthy();

    // Unbind and then delete
    await directExchange.unbindQueue(queueB, exchangeA, rkB);

    await directExchange.delete(exchangeA);
  });
  ...
  it('bindQueue: rejects invalid routing keys', async () => {
    ...
    for (const k of invalidKeys) {
      await expect(
        directExchange.bindQueue(queueA, exchangeA, k),
  ...
  it('matchQueues: rejects invalid routing keys', async () => {
    ...
    for (const k of invalidKeys) {
      await expect(directExchange.matchQueues(exchangeA, k)).rejects.toThrow(
        InvalidExchangeRoutingKeyError,
      );
    }
  });
  ...
  it('unbindQueue: rejects invalid routing keys', async () => {
    ...
    for (const k of invalidKeys) {
      await expect(
        directExchange.unbindQueue(queueA, exchangeA, k),
      ).rejects.toThrow(InvalidDirectExchangeParametersError);
    }

    // Ensure valid unbind still works
    await expect(
      directExchange.unbindQueue(queueA, exchangeA, rkA),
  ...
  it('accepts valid routing keys (alphanumeric, hyphens, underscores, dots)', async () => {
    ...
    for (const k of validKeys) {
      ...
      const matched = await directExchange.matchQueues(exchangeA, k);
      expect(matched).toEqual([queueA]);

      // Clean up for the next key
      await directExchange.unbindQueue(queueA, exchangeA, k);
    }
  });
  ...
  it('binding with different valid keys does not leak between keys', async () => {
    const k1 = 'alpha.key';
    const k2 = 'beta_key';
    await directExchange.bindQueue(queueA, exchangeA, k1);
    await directExchange.bindQueue(queueB, exchangeA, k2);

    const m1 = await directExchange.matchQueues(exchangeA, k1);
    const m2 = await directExchange.matchQueues(exchangeA, k2);

    expect(m1).toEqual([queueA]);
    expect(m2).toEqual([queueB]);

    await directExchange.unbindQueue(queueA, exchangeA, k1);
    await directExchange.unbindQueue(queueB, exchangeA, k2);
  });
  ...
  it('getRoutingKeys: returns all routing keys bound to the exchange (unique set)', async () => {
    ...
    const keys = await directExchange.getRoutingKeys(exchangeA);
  ...
  it('getRoutingKeys: reflects removal only after the last queue unbinds from a key', async () => {
    ...
    await directExchange.bindQueue(queueA, exchangeA, rkA);
    await directExchange.bindQueue(queueB, exchangeA, rkA);

    // Keys should contain rkA
    let keys = await directExchange.getRoutingKeys(exchangeA);
    expect(keys).toEqual([rkA]);

    // Unbind one queue -> rkA should still be present
    await directExchange.unbindQueue(queueA, exchangeA, rkA);
    keys = await directExchange.getRoutingKeys(exchangeA);
    expect(keys).toEqual([rkA]);

    // Unbind the last queue -> rkA should be removed
    await directExchange.unbindQueue(queueB, exchangeA, rkA);
    keys = await directExchange.getRoutingKeys(exchangeA);
    expect(keys).toEqual([]);
  });
  ...
  it('getRoutingKeyQueues: rejects invalid routing keys', async () => {
    ...
    for (const k of invalidKeys) {
      await expect(
        directExchange.getRoutingKeyBoundQueues(exchangeA, k),
      ).rejects.toThrow(InvalidDirectExchangeParametersError);
    }
  });
});
