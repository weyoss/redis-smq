...
test('Combined test: Delete acknowledged messages by IDs which include a non-existent message ID', async () => {
  ...
  const count = await acknowledgedMessages.countMessages(defaultQueue);
  expect(count).toBe(2);

  const message = await getMessageManager();
  const reply = await message.deleteMessagesByIds([msg1]);
  ...
  const count1 = await acknowledgedMessages.countMessages(defaultQueue);
  expect(count1).toBe(1);

  const reply2 = await message.deleteMessagesByIds([msg1, msg2]);
  ...
  const count2 = await acknowledgedMessages.countMessages(defaultQueue);
